...
毎週水曜の「生バンドカラオケ」の告知を、**毎週金曜11時に Instagram へ自動で投稿**します。
さらに**開催当日（水曜）の朝に、Instagram のストーリーをもう一度**出します。

- Instagram：フィード（2枚のカルーセル）＋ ストーリーズ
- **Facebook は手で貼ります。**自動では投稿できません
  （[Facebookへの貼り方.md](Facebookへの貼り方.md)）
...
木曜11時   下書きが GitHub の Issue に届きます（メールも来ます）
        ...
        （メールにそのまま返信してもかまいません）
             ↓
金曜11時   告知を投稿（Instagram のフィードとストーリー）
        結果が同じ Issue にコメントされ、Issue が閉じます
             ↓
水曜の朝   当日のおしらせ（Instagram のストーリーだけ）
        「本日◯月◯日！」「本日のホストメンバー」の画像に差し替わります
```

**`approve` は1回だけです。** 金曜の告知と、当日のおしらせの両方に効きます。
...
当日のおしらせは、**朝5時台から8時台にかけて何回か**出そうとします。
GitHub の定時実行は混んでいると遅れるので、**8時までに必ず出る約束はできません**。
時刻をきっちり決めたいときは、Actions の「水曜の朝 当日のおしらせを出す」から
...
| `scripts/make_images.py` | 開催日とホスト名を画像に焼き込む（告知用と当日用の2組） |
| `scripts/post.py` | Instagram と Facebook に投稿する |
| `scripts/already_posted.py` | もう出した回かどうかを見て、二度出しを防ぐ |
...
- **Facebook には自動で投稿できません。** Meta から `pages_manage_posts` の許可が
  下りていないためです（2026-09-17 に確定）。`data/config.json` の `facebook_feed` と
  `facebook_story` を `false` にしてあります。手で貼るやり方は
  [Facebookへの貼り方.md](Facebookへの貼り方.md) にあります。
